...
async def cancel_session(mid):
    ...
    for pid in s.get("party_ids", []):
        if pid.startswith("ai_party_"):
            ...
            # ★ match_id / assigned_team 을 완전히 비우고 searching 으로 복원
            await rtdb_patch(f"match_queue/parties/{pid}", {
                "match_id":      "",
                "assigned_team": "",
                "status":        "searching"
    ...
    try:
        while True:
            ...
            if t == "q":
                ...
                # ★ 매칭 요청 시 해당 파티를 무조건 searching 으로 강제 리셋
                # (이전 매치의 matched/loading 찌꺼기 제거)
                if pid:
                    await rtdb_patch(f"match_queue/parties/{pid}", {
                        "status":        "searching",
                        "match_id":      "",
                    ...
                    print(f"[q] 파티 {pid} → searching 강제 리셋", flush=True)
                else:
                    ...
                    if all_parties and isinstance(all_parties, dict):
                        for fpid, fp in all_parties.items():
                            if uid in fp.get("members", {}) and not fpid.startswith("ai_party_"):
                                ...
                                print(f"[q] 파티 {fpid} → searching 리셋 (순회)", flush=True)
                                break
